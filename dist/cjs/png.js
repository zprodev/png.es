...
function parse(input, oprion) {
    ...
    const rawData = (oprion && oprion.inflate) ? oprion.inflate(idat.data) : zlib_es.inflate(idat.data);
...
function pack(png, oprion) {
    ...
    const idatData = (oprion && oprion.deflate) ? oprion.deflate(filterData) : zlib_es.deflate(filterData);
    chunks.set('IDAT', {
        type: 'IDAT',
        data: idatData,
    });
